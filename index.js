...
async function main() {
	{
		...
		Promise.all(marketPresentationResult[1]).then((value) => {
			for (let i = 0; i < marketPresentationResult[0].length; i++) {
				log(marketPresentationResult[0][i] + ': ' + value[i].toFixed(2) + '€');
			}
		});
	}
	...
	log('bnb is associated to : ');
	for (let i = 0; i < symbolName.length; i++) {
		log('	name ' + symbolName[i] + ' => with id ' + symbolId[i]);
	}
	...
	log('ishares-msci-world-etf-tokenized-stock-defichain is associated to : ');
	for (let i = 0; i < idName.length; i++) {
		log('	name ' + idName[i] + ' => with symbol ' + idSymbol[i]);
	}

	const nameId = await nameToId('0.5X Long Dogecoin', client);
	const nameSymbol = await nameToSymbol('0.5X Long Dogecoin', client);
	log('0.5X Long Dogecoin is associated to : ');
	for (let i = 0; i < nameId.length; i++, client) {
		log('	id ' + nameId[i] + ' => with symbol ' + nameSymbol[i]);
	}
	...
	for (let i = 0; i < result.length; i++) {
		...
		log('the ' + i + ' result is : ' + Element.name);
		log(Element);
	}
